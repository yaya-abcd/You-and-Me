...
async function handleLogin(event) {
  ...
  try {
    ...
    await validateSyncAccess();
    await unlockPrivatePhotos(passcode);
...
async function unlockPrivatePhotos(passcode) {
  releasePrivatePhotoUrls();
  builtInPhotos = [];

  const manifestResponse = await fetch(PRIVATE_MANIFEST_URL, { cache: "no-store" });
  if (!manifestResponse.ok) throw new Error("private photo manifest unavailable");

  const manifest = await manifestResponse.json();
  if (manifest.version !== 1 || manifest.cipher !== "AES-GCM") {
    throw new Error("unsupported private photo format");
  }
  ...
  const key = await crypto.subtle.deriveKey(
    {
      ...
      salt: base64ToBytes(manifest.kdf.salt),
  ...
  try {
    for (const photo of manifest.photos) {
      const photoUrl = new URL(photo.file, manifestResponse.url).href;
      const response = await fetch(photoUrl, { cache: "no-store" });
      if (!response.ok) throw new Error(`private photo unavailable: ${photo.id}`);
      ...
      decryptedPhotos.push({ src: url, caption: photo.caption });
    }
  } catch (error) {
    releasePrivatePhotoUrls();
    throw error;
...
async function validateSyncAccess() {
  if (!isSyncApiConfigured()) throw new Error("sync API is not configured");
  await syncRequest("/unlock", { method: "POST" });
}
