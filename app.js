...
function renderAlbum(photos) {
  ...
  if (!photos.length) {
    els.albumGrid.append(emptyState("把喜欢的照片加入相处中吧"));
    return;
  }
...
function photoTile(photo, allPhotos) {
  ...
  deleteButton.addEventListener("click", async () => {
    if (!await confirmDelete("删除这张照片？", "照片将从相处中和循环展示中移除。")) return;
