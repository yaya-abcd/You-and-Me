...
const els = {
  ...
  wishDetailDialog: document.querySelector("#wishDetailDialog"),
  closeWishDetail: document.querySelector("#closeWishDetail"),
  wishDetailText: document.querySelector("#wishDetailText"),
  wishCreatedTime: document.querySelector("#wishCreatedTime"),
  wishCompletedRow: document.querySelector("#wishCompletedRow"),
  wishCompletedTime: document.querySelector("#wishCompletedTime"),
  wishDetailStatus: document.querySelector("#wishDetailStatus"),
  wishProgressCount: document.querySelector("#wishProgressCount"),
  wishProgressList: document.querySelector("#wishProgressList"),
  wishProgressForm: document.querySelector("#wishProgressForm"),
  wishProgressText: document.querySelector("#wishProgressText"),
  wishProgressPhotoInput: document.querySelector("#wishProgressPhotoInput"),
  wishProgressPhotoLabel: document.querySelector("#wishProgressPhotoLabel"),
  wishProgressPhotoWrap: document.querySelector("#wishProgressPhotoWrap"),
  wishProgressPhotoPreview: document.querySelector("#wishProgressPhotoPreview"),
  clearWishProgressPhoto: document.querySelector("#clearWishProgressPhoto"),
  saveWishProgress: document.querySelector("#saveWishProgress"),
...
let activeWishDetailId = null;
let wishProgressPhoto = null;
let wishProgressPhotoUrl = "";
...
function bindEvents() {
  ...
  els.wishProgressForm.addEventListener("submit", saveWishProgress);
  els.wishProgressPhotoInput.addEventListener("change", prepareWishProgressPhoto);
  els.clearWishProgressPhoto.addEventListener("click", clearWishProgressPhoto);
  els.closeWishDetail.addEventListener("click", closeWishDetail);
  els.wishDetailDialog.addEventListener("close", clearWishProgressDraft);
...
function renderWishes(wishes) {
  ...
  sorted.forEach((wish) => {
    ...
    text.type = "button";
    text.className = "wish-text-button";
    text.textContent = wish.text;
    text.title = "查看愿望时间";
    text.addEventListener("click", () => openWishDetail(wish));
...
function openWishDetail(wish) {
  activeWishDetailId = wish.id;
  const completedAt = wish.completedAt || (wish.done ? wish.updatedAt : null);
  els.wishDetailText.textContent = wish.text;
  els.wishCreatedTime.textContent = formatExactDateTime(wish.createdAt);
  els.wishDetailStatus.textContent = wish.done ? "这个愿望已经实现" : "这个愿望正在等待实现";
  els.wishCompletedRow.classList.toggle("is-hidden", !wish.done);
  els.wishCompletedTime.textContent = wish.done && completedAt ? formatExactDateTime(completedAt) : "";
  renderWishProgress(wish);
  if (!els.wishDetailDialog.open) els.wishDetailDialog.showModal();
}

function closeWishDetail() {
  activeWishDetailId = null;
  els.wishDetailDialog.close();
}

function renderWishProgress(wish) {
  const entries = [...(wish.progress || [])].sort((a, b) => a.createdAt - b.createdAt);
  els.wishProgressCount.textContent = `${entries.length} 条记录`;
  els.wishProgressList.replaceChildren();

  if (!entries.length) {
    const empty = document.createElement("p");
    empty.className = "wish-progress-empty";
    empty.textContent = "还没有过程记录，从这一刻开始写下吧。";
    els.wishProgressList.append(empty);
    return;
  }

  entries.forEach((entry) => {
    const item = document.createElement("article");
    item.className = "wish-progress-item";
    const header = document.createElement("header");
    const meta = document.createElement("span");
    meta.textContent = `${ACCOUNTS[entry.author]?.label || entry.author} · ${formatExactDateTime(entry.createdAt)}`;
    const removeButton = document.createElement("button");
    removeButton.type = "button";
    removeButton.className = "icon-delete";
    removeButton.textContent = "删除";
    removeButton.addEventListener("click", async () => {
      if (!await confirmDelete("删除这条实现记录？", "记录中的文字和照片都会被删除。")) return;
      try {
        await mutateRemoteState((state) => {
          const target = state.wishes.find((item) => item.id === wish.id);
          if (target) target.progress = (target.progress || []).filter((item) => item.id !== entry.id);
        }, `Delete wish progress by ${activeAccount}`);
        const latest = remoteState.wishes.find((item) => item.id === wish.id);
        if (latest && activeWishDetailId === wish.id) openWishDetail(latest);
        showToast("实现记录已删除");
      } catch (error) {
        showToast("删除失败，原记录仍然保留");
      }
    });
    ...
    if (entry.imageBlob) {
      const image = document.createElement("img");
      const url = objectUrl(entry.imageBlob);
      image.src = url;
      image.alt = entry.imageName || "愿望实现过程照片";
      image.addEventListener("click", () => openPhoto(url, wish.text));
      item.append(image);
    }
    els.wishProgressList.append(item);
  });
}

async function saveWishProgress(event) {
  event.preventDefault();
  const wishId = activeWishDetailId;
  const text = els.wishProgressText.value.trim();
  const photo = wishProgressPhoto;
  if (!wishId || (!text && !photo)) {
    showToast("写点过程或添加一张照片");
    return;
  }

  els.saveWishProgress.disabled = true;
  els.saveWishProgress.textContent = "保存中...";
  const entry = {
    id: crypto.randomUUID(),
    author: activeAccount,
    text,
  ...
  try {
    await mutateRemoteState((state) => {
      ...
      if (!wish) throw new Error("wish unavailable");
      wish.progress = [...(wish.progress || []), entry];
      wish.updatedAt = Date.now();
    }, `Add wish progress by ${activeAccount}`);
    els.wishProgressText.value = "";
    clearWishProgressPhoto();
    const latest = remoteState.wishes.find((item) => item.id === wishId);
    if (latest && activeWishDetailId === wishId) openWishDetail(latest);
    showToast("实现过程已保存");
  } catch (error) {
    showToast("记录保存失败，内容仍然保留");
  } finally {
    els.saveWishProgress.disabled = false;
    els.saveWishProgress.textContent = "保存记录";
  }
}

async function prepareWishProgressPhoto() {
  const file = els.wishProgressPhotoInput.files[0];
  if (!file) {
    clearWishProgressPhoto();
    return;
  }

  els.wishProgressPhotoInput.disabled = true;
  els.wishProgressPhotoLabel.textContent = "正在压缩...";
  try {
    const compressed = await compressImageFile(file);
    if (compressed.size > MAX_ATTACHMENT_BYTES) throw new Error("photo too large");
    wishProgressPhoto = compressed;
    if (wishProgressPhotoUrl) URL.revokeObjectURL(wishProgressPhotoUrl);
    wishProgressPhotoUrl = URL.createObjectURL(compressed);
    els.wishProgressPhotoPreview.src = wishProgressPhotoUrl;
    els.wishProgressPhotoWrap.classList.remove("is-hidden");
    els.wishProgressPhotoLabel.textContent = "更换照片";
  } catch (error) {
    clearWishProgressPhoto();
    showToast(error.message === "photo too large" ? "照片压缩后仍超过 8MB" : "照片处理失败，请重试");
  } finally {
    els.wishProgressPhotoInput.disabled = false;
  }
}

function clearWishProgressPhoto() {
  wishProgressPhoto = null;
  els.wishProgressPhotoInput.value = "";
  els.wishProgressPhotoPreview.removeAttribute("src");
  els.wishProgressPhotoWrap.classList.add("is-hidden");
  els.wishProgressPhotoLabel.textContent = "添加照片";
  if (wishProgressPhotoUrl) URL.revokeObjectURL(wishProgressPhotoUrl);
  wishProgressPhotoUrl = "";
}

function clearWishProgressDraft() {
  els.wishProgressText.value = "";
  clearWishProgressPhoto();
}
...
async function serializeWishes(wishes) {
  return Promise.all(wishes.map(async (wish) => ({
    ...wish,
    progress: await serializeBlobs(wish.progress || []),
  })));
}

async function deserializeWishes(wishes) {
  return Promise.all(wishes.map(async (wish) => ({
    ...wish,
    progress: await deserializeBlobs(wish.progress || []),
  })));
}
...
async function encryptRemoteState(state = remoteState) {
  ...
  const serializable = {
    ...
    wishes: await serializeWishes(state.wishes),
...
async function decryptRemoteState(bytes) {
  ...
  return {
    ...
    wishes: await deserializeWishes(parsed.wishes || []),
